...
<form wire:submit.prevent="save" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="px-4 py-2">
            ...
            <x-input wire:model="user.name" type="text" name="name" :value="old('name')"
        ...
        <div class="px-4 py-2">
            ...
            <x-input wire:model="user.uname" type="text" name="uname" :value="old('uname')"
                class="w-full" required />
            @error('user.uname')
                <div class="text-red-600">{{ $message }}</div>
            @enderror
        ...
        <div class="px-4 py-2">
            ...
            <x-input wire:model="user.email" type="email" name="email" :value="old('email')"
                class="w-full" required />
            @error('user.email')
                <div class="text-red-600">{{ $message }}</div>
            @enderror
        ...
        <div class="px-4 py-2">
            ...
            <x-input wire:model="user.phone_number" class="w-full" type="number" name="phone_number"
                :value="old('phone_number', '62')" required />
        ...
        <div class="px-4 py-2">
            ...
            <select wire:model="user.position"
                ...
                <option value="" class="text-slate-400"
                    {{ is_null($this->user->position) ? 'selected' : '' }}>Select an option</option>
                    <option value="Employee"
                        {{ $this->user->position == "Employee" ? 'selected' : '' }}>
                        Employee
                    </option>
                    <option value="Marketing"
                        {{ $this->user->position == "Marketing" ? 'selected' : '' }}>
                        Marketing
                    </option>
                    ...
                    <option value="Manager"
                        {{ $this->user->position == "Manager" ? 'selected' : '' }}>
                        Manager
                    </option>
                    <option value="Director"
                        {{ $this->user->position == "Director" ? 'selected' : '' }}>
                        Director
                    </option>
                    <option value="Supervisor PPIC"
                        {{ $this->user->position == "Supervisor PPIC" ? 'selected' : '' }}>
                        Supervisor PPIC
                    </option>
            </select>
            @error('user.position')
